...
class Connector:
    ...
    def _select_baud_rate(self):
        ...
        while True:
            ...
            if 0 <= index < len(self.BAUD_RATES):
                self._logger.info(f"Selected baud rate: {self.BAUD_RATES[index]}")
                return [self.BAUD_RATES[index]]
            # User chose to try all baud rates
            elif index == len(self.BAUD_RATES):
                self._logger.info("Selected option to try all baud rates.")
                return self.BAUD_RATES
            # User cancelled
            elif index == len(self.BAUD_RATES) + 1:
                self._logger.info("Baud rate selection aborted.")
                return None
            # Invalid selection
            else:
                self._logger.info("Invalid selection! Enter a number from the list.")
    ...
    def _handle_manual_port_selection(self, ports):
        ...
        while True:
            ...
            try:
                ...
                self._logger.info("Invalid input. Please enter a number from the list.")
                continue

            if 0 <= port_index < len(ports):
                self._logger.info(f"Selected port: {ports[port_index].device} - {ports[port_index].description}")
                return ports[port_index].device
            elif port_index == len(ports):
                self._logger.info("Cancelled. Returning to main menu.")
                return None
            else:
                self._logger.info("Invalid port selection. Please try again.")
    ...
    def _handle_prolific_port_search(self, ports):
        ...
        self._logger.info("No Prolific USB serial adapter ports found.")
        return None
        ...
        while True:
            ...
            while True:
                ...
                if choice not in ["1", "2", "3", "4"]:
                    self._logger.info("Invalid choice. Please enter 1, 2, 3, or 4.")
                    continue

                # Valid choice - process accordingly
                if choice == "1":
                    self._logger.info("Selected option to manually select a port.")
                    selected_port = self._handle_manual_port_selection(ports)
                    selected_ports = [selected_port] if selected_port else None
                elif choice == "2":
                    self._logger.info("Selected option to attempt all available ports.")
                    selected_ports = self._handle_all_ports_selection(ports)
                elif choice == "3":
                    self._logger.info("Selected option to search for Prolific USB serial adapter ports.")
                    selected_ports = self._handle_prolific_port_search(ports)
                elif choice == "4":
                    self._logger.info("Connection process aborted by user.")
                    return False
    ...
    def connect(self):
        ...
        while True:
            ...
            if retry not in ['y', 'yes']:
                self._logger.info("User has chosen not to retry connection. Exiting connection process.")
                return None
            else:
                self._logger.info("User has chosen to retry connection.")
