...
class OregonConnector:
    ...
    def _select_baud_rate(self):
        ...
        self._logger.info(selection_options, extra=logging_extra)
        ...
        while True:
            ...
            try:
                ...
                self._logger.info("Invalid selection! Enter a number from the list.", extra=logging_extra)
                continue
            ...
            if 0 <= index < len(self.BAUD_RATES):
                self._logger.info(f"Selected baud rate: {self.BAUD_RATES[index]}", extra=logging_extra)
                return [self.BAUD_RATES[index]]
            # User chose to try all baud rates
            elif index == len(self.BAUD_RATES):
                self._logger.info("Selected option to try all baud rates.", extra=logging_extra)
                return self.BAUD_RATES
            # User cancelled
            elif index == len(self.BAUD_RATES) + 1:
                self._logger.info("Baud rate selection aborted.", extra=logging_extra)
                return None
            # Invalid selection
            else:
                self._logger.info("Invalid selection! Enter a number from the list.", extra=logging_extra)
    ...
    def _handle_manual_port_selection(self, ports):
        ...
        if not ports:
            self._logger.info("No ports available.", extra=logging_extra)
            return None
        ...
        while True:
            ...
            try:
                ...
                self._logger.info("Invalid input. Please enter a number from the list.", extra=logging_extra)
                continue

            if 0 <= port_index < len(ports):
                self._logger.info(f"Selected port: {ports[port_index].device} - {ports[port_index].description}", extra=logging_extra)
                return ports[port_index].device
            elif port_index == len(ports):
                self._logger.info("Cancelled. Returning to main menu.", extra=logging_extra)
                return None
            else:
                self._logger.info("Invalid port selection. Please try again.", extra=logging_extra)
    ...
    def _handle_prolific_port_search(self, ports):
        ...
        if prolific_ports:
            self._logger.info(f"Found Prolific USB serial adapter port(s): {prolific_ports}", extra=logging_extra)
            return prolific_ports

        self._logger.info("No Prolific USB serial adapter ports found.", extra=logging_extra)
        return None
    ...
    def _handle_all_ports_selection(self, ports):
        ...
        self._logger.info(f"Each available port will be attempted: {port_devices}", extra=logging_extra)
        return port_devices
        ...
        while True:
            ...
            while True:
                ...
                if choice not in ["1", "2", "3", "4"]:
                    self._logger.info("Invalid choice. Please enter 1, 2, 3, or 4.", extra=logging_extra)
                    continue

                # Valid choice - process accordingly
                if choice == "1":
                    self._logger.info("Selected option to manually select a port.", extra=logging_extra)
                    selected_port = self._handle_manual_port_selection(ports)
                    selected_ports = [selected_port] if selected_port else None
                elif choice == "2":
                    self._logger.info("Selected option to attempt all available ports.", extra=logging_extra)
                    selected_ports = self._handle_all_ports_selection(ports)
                elif choice == "3":
                    self._logger.info("Selected option to search for Prolific USB serial adapter ports.", extra=logging_extra)
                    selected_ports = self._handle_prolific_port_search(ports)
                elif choice == "4":
                    self._logger.info("Connection process aborted by user.", extra=logging_extra)
                    return False
